...
with tf.variable_scope('inputs'):
    x = tf.placeholder(tf.float32, [None, X.shape[1]], name = 'x_input')
    y = tf.placeholder(tf.float32, [None, Y.shape[1]], name = 'y_input')


with tf.device('/gpu:0'):
    l1 = add_layer(x, X.shape[1], 2048, 1, tf.nn.sigmoid)
  ...
  # Restore variables from disk.
    ...
    for i in xrange(args.num_rollouts):
        ...
        while not done:
            ...
            if steps >= max_steps:
                break
    nobservations = np.array(observations).reshape(len(observations), 376)
    nactions = np.array(actions).reshape(len(actions), 17)
    
    X = np.concatenate((X, nobservations), axis = 0)
    Y = np.concatenate((Y, nactions), axis = 0)
    #modified to get cost function
    expert_data = {'observations': X,
                   'actions': Y}
    with open('cost.pkl','wb') as f:
        f.truncate()
    pickle.dump(expert_data, open("cost.pkl", "wb"))
