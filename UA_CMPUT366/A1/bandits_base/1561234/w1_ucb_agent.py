...
def agent_step(reward, this_observation): # returns NumPy array, reward: floating point, this_observation: NumPy array
    ...
    atp1 = np.argmax(Q_value + C * np.sqrt(1. * math.log(step)/N_action))
    local_action[0] = atp1

    apt2 = int(last_action[0])
    # might do some learning here
    N_action[apt2] += 1
    Q_value[apt2] += 1./N_action[apt2]*(reward - Q_value[apt2])
...
def agent_cleanup():
    ...
    Q_value = np.zeros(num_actions) #5 * np.ones(num_actions)
    N_action = np.zeros(num_actions)
    step = 0
    return
